...
- (void)sendMessageWithData {
    ...
    self.sendMessageTimer = [NSTimer scheduledTimerWithTimeInterval:4 target:self selector:@selector(sendMessageWithData) userInfo:nil repeats:YES];
...
//        NSLog(@"%@", [NSDate date]);
        ...
        NSLog(@"%@", timeLocal);
        [kNotificationCenter postNotificationName:kSocketData object:data];
    }
    [self.asyncSocket readDataWithTimeout:-1 tag:200];
}
